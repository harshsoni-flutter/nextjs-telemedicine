...
export default function SignUpProviderPage() {
  ...
  async function onSubmit(data: FormData) {
    ...
    const result = await signUpProvider(formData);
    if (result?.error) {
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <Card className="border-[#e8e6e1] bg-white shadow-sm">
          <CardHeader className="space-y-1 text-center">
            <CardTitle className="text-2xl text-[#1a1f2e]">Provider sign up</CardTitle>
            <CardDescription className="text-[#5c6370]">
              Create an account to offer telemedicine appointments
            </CardDescription>
          </CardHeader>
